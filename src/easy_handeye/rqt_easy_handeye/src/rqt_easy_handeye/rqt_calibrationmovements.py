...
class CalibrationMovementsGUI(QWidget):
    ...
    def __init__(self):
        ...
        self.start_auto_calibrate_btn = QPushButton('Start Calibrate Automatically')
        self.start_auto_calibrate_btn.clicked.connect(self.handle_calibrate_automatically)
        ...
        self.buttons_layout.addWidget(self.start_auto_calibrate_btn)
    ...
    def handle_calibrate_automatically(self):
        self.state = CalibrationMovementsGUI.CHECKING_STARTING_POSITION
        self.update_ui()
        res = self.handeye_client.check_starting_pose()
        if res.can_calibrate:
        ...
        for i in range(len(self.target_poses)):
            res_nextpose = self.handeye_client.select_target_pose(self.current_target_pose+1)
            self.current_target_pose = res_nextpose.target_poses.current_target_pose_index
            self.target_poses = res_nextpose.target_poses.target_poses
            res_plan = self.handeye_client.plan_to_selected_target_pose()
            self.plan_was_successful = res_plan.success
            if self.plan_was_successful:
                self.state = CalibrationMovementsGUI.GOOD_PLAN
                res_execute = self.handeye_client.execute_plan()
                if res_execute.success:
        ...
        if result.valid:
            self.guide_lbl.setText(str(result.calibration.transform.transform))
            print(str(result.calibration.transform.transform))
